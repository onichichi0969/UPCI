...
namespace UPCI.Portal.Pages.Reports.LogHTTP
{
    public class LogHTTPModel : PageModel
    {
        ...
        public LogHTTPModel(IConfiguration configuration, IReportService reportService)
        {
        ...
        public JsonResult OnPostFilter([FromBody] FParam fparam)
        {
            ...
            var result = _reportService.LogHTTP(fparam).Result; 
        ...
        public IActionResult OnPostDownload([FromBody] FParam fparam)
        {
            ...
            string fileName = "Report_Log_HTTP (" + DateTime.UtcNow.ToString("MM-dd-yyyy") + ").xlsx";
            fparam.OpUser = HttpContext.Session.GetString("Username");
            fparam.Terminal = HttpContext.Session.GetString("Terminal");
            var result = _reportService.LogHTTPListOnly(fparam).Result;
            try
            {
                using (ExcelPackage package = new ExcelPackage())
                {
                    ...
                    var properties = typeof(UPCI.DAL.DTO.Response.ReportLogHTTP).GetProperties(BindingFlags.Public | BindingFlags.Instance);
                    // Create header row
