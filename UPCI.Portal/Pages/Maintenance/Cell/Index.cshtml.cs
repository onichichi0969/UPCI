...
namespace UPCI.Portal.Pages.Maintenance.Cell
{
    public class IndexModel : PageModel
    {
        ...
        public IndexModel(IConfiguration configuration, ICellService cellService)
        {
        ...
        { 
            var items = _cellService.Get().Result;
        ...
        public JsonResult OnPostFilter([FromBody] FParam fparam)
        {
            ...
            var items = _cellService.Filter(fparam).Result;

            return new JsonResult(items);
        }

        public async Task<JsonResult> OnPostSave([FromBody] UPCI.DAL.DTO.Request.Cell model)
        {
            ...
            if (model.Id.Trim() != "")
                result = await _cellService.Update(model);
            else
                result = await _cellService.Create(model);
        ...
        public async Task<JsonResult> OnPostDelete([FromBody] UPCI.DAL.DTO.Request.Cell model)
        {
            ...
            model.Terminal = HttpContext.Session.GetString("Terminal");
            var result = new DAL.DTO.Response.Result();
            result = await _cellService.Delete(model);
