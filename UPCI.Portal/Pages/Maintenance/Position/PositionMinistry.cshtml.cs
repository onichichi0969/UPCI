...
namespace UPCI.Portal.Pages.Maintenance.Position.PositionMinistry
{
    public class PositionMinistryModel : PageModel
    {
        ...
        public PositionMinistryModel(IConfiguration configuration, IPositionService positionService)
        {
        ...
        { 
            var items = _positionService.GetMinistry().Result;
        ...
        public JsonResult OnPostFilter([FromBody] FParam fparam)
        {
            ...
            var items = _positionService.FilterMinistry(fparam).Result;

            return new JsonResult(items);
        }

        public async Task<JsonResult> OnPostSave([FromBody] UPCI.DAL.DTO.Request.PositionMinistry model)
        {
            ...
            if (model.Id.Trim() != "")
                result = await _positionService.UpdateMinistry(model);
            else
                result = await _positionService.CreateMinistry(model);

            return new JsonResult(result);
        }
        public async Task<JsonResult> OnPostDelete([FromBody] UPCI.DAL.DTO.Request.PositionMinistry model)
        {
            model.OpUser = HttpContext.Session.GetString("Username");
            model.Terminal = HttpContext.Session.GetString("Terminal");
            var result = new DAL.DTO.Response.Result();
            result = await _positionService.DeleteMinistry(model);
