...
namespace UPCI.Portal.Pages.Maintenance.Company
{
    public class IndexModel : PageModel
    {
        ...
        public IndexModel(IConfiguration configuration, ICompanyService companyService)
        {
        ...
        { 
            var items = _companyService.Get().Result;
        ...
        public JsonResult OnPostFilter([FromBody] FParam fparam)
        {
            ...
            var items = _companyService.Filter(fparam).Result;

            return new JsonResult(items);
        }

        public async Task<JsonResult> OnPostSave([FromBody] UPCI.DAL.DTO.Request.Company model)
        {
            ...
            if (model.Id.Trim() != "")
                ...
                result = await _companyService.Create(model);

            return new JsonResult(result);
        }
        public async Task<JsonResult> OnPostDelete([FromBody] UPCI.DAL.DTO.Request.Company model)
        {
            model.OpUser = HttpContext.Session.GetString("Username");
            model.Terminal = HttpContext.Session.GetString("Terminal");
            var result = new DAL.DTO.Response.Result();
